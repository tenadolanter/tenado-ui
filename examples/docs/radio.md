...
:::demo 只要在`td-radio`元素中设置`disabled`属性即可，它接受一个`Boolean`，`true`为禁用。
...
:::demo 结合`td-radio-group`元素和子元素`td-radio`可以实现单选组，在`td-radio-group`中绑定`v-model`，在`td-radio`中设置好`label`即可，无需再给每一个`td-radio`绑定变量，另外，还提供了`change`事件来响应变化，它会传入一个参数`value`。
...
:::demo 只需要把`td-radio`元素换成`td-radio-button`元素即可，此外，Element 还提供了`size`属性。
