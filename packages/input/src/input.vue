<template>
  ...
  >
    <template v-if="type !== 'textarea'">
      <!-- 前置元素 -->
      <div class="td-input-group__prepend" v-if="$slots.prepend">
      ...
      />
      <!-- 前置内容 -->
      <span class="td-input__prefix" v-if="$slots.prefix || prefixIcon">
        <slot name="prefix"></slot>
        <i class="td-input__icon" v-if="prefixIcon" :class="prefixIcon"> </i>
      </span>
      <!-- 后置内容 -->
      ...
      <!-- 后置元素 -->
      <div class="td-input-group__append" v-if="$slots.append">
...
export default {
  ...
  computed: {
    ...
    validateIcon() {
      return {
        validating: "td-icon-loading",
        success: "td-icon-circle-check",
        error: "td-icon-circle-close",
      }[this.validateState];
    },
    textareaStyle() {
  ...
  methods: {
    ...
    calcIconOffset(place) {
      let elList = [].slice.call(
        this.$el.querySelectorAll(`.td-input__${place}`) || []
      );
      ...
      if (this.$slots[pendant]) {
        el.style.transform = `translateX(${place === "suffix" ? "-" : ""}${
          this.$el.querySelector(`.td-input-group__${pendant}`).offsetWidth
        }px)`;
      } else {
